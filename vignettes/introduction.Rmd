...
---
<!-- suppress-bibliography: true -> unterdrückt die Bibiliografie  -->
<!-- Am Ende vom Blatt muss wieder # References eingefügt werden  -->
...
accuracy and F1 Score, the harmonic mean of precision and recall, of the 
algorithms performances. However, for each test or plot one specific measure 
column has to be defined since it is not possible to  compare multiple measures 
at once. 
...
non-parametric test that ranks and compares the performances of two classifiers, based on their ranks.
...
which proposed the use of Bayesian tests for comparing the 
...
| 2 algorithms x 1 problem set | 2 algortihms x n problem sets |
|:------:|:------:|
...
| algo_1 >= algo_2 if right + rope probability > 0.95  | 
| algo_1 <= algo_2 if left + rope probability > 0.95   | 
...
```{r, prompt=TRUE , fig.width=5.5, fig.height=3.5, message = FALSE}
...
The hierarchical approach is recommended by @benavoli2017 over the 
...
comparison. The procedure of assessing the algorithms performances is mostly time-consuming. To counteract this issue this package provides a 
...
The sequential procedure can be outlined as follows: 
...
The example below shows how the function `get_replications` can easily be 
replaced by a self-written one. The user can use this to evaluate the applied 
algorithms within the test procedure.  
...
assignInNamespace("get_replication", sepcific_get_replication, ns="seqbtests", envir=as.environment("package:seqbtests"))
assign("get_replication", 
  sepcific_get_replication, envir=as.environment("package:seqbtests"))
