...
The common practice in frequentict hypothesis testing is to declare a significance level of p <= 0.05. If the p-value is below this threshold the accuracy is significantly different on that problem set. 

If no algorithm for comparing is defined, the baseline is tested against all algorithms in the data frame. Note that one has to take the multiple testing problem in account for the frequentist hypothesis tests and may correct the significance levlel via Bonferroni correction or other correctiom procedures. 
### Friedman test 
The friedman test is a non-parametric test that checks for differences among the algorithms performances. It has been recommended by Demsar [see @demsar2006] together with the corresponding post-hoc tests for comparison of classifiers among multiole problem sets. 
It is the non-parametric equivalent to the ANOVA test. The friedman test ranks the algorithms for each problemset seperately. 



 hier noch den CD plot
