...
x <- matrix(data_wide[,  learner_a], byrow=TRUE, nrow=length(unique(data_wide$replications)))
y <- matrix(data_wide[,  learner_b], byrow=TRUE, nrow=length(unique(data_wide$replications)))
...
results <- b_hierarchical_test(df = test_benchmark, learner_a = "algo_1", learner_b = "algo_2")
results
